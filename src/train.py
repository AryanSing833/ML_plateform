#!/usr/bin/env python3
"""
Training script for Pytorch classification model
"""
...
def create_data_loaders(X_train: np.ndarray, y_train: np.ndarray, 
    ...
    y_train_tensor = torch.LongTensor(y_train) if "classification" == "classification" else torch.FloatTensor(y_train)
    X_test_tensor = torch.FloatTensor(X_test)
    y_test_tensor = torch.LongTensor(y_test) if "classification" == "classification" else torch.FloatTensor(y_test)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train Pytorch classification model")
    ...
    with mlflow.start_run():
        ...
        model = ClassificationModel(model_config).to(device)
        ...
        # Save model
        model_path = "models/production/classification_model.pth"
